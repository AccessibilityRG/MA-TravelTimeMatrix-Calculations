...
for block in range(iterations):
    ...
    outName = os.path.join(outFolder, "%s%s" % (name_idx, filename))    # note! The "%03d" % (name_idx)  -combination generates 3-gidit running numbers
...
#-------------------------------------
# Parse the commands into .sh files
#-------------------------------------

allCommands=open(os.path.join(batch_folder, cmd_file), 'r')

batch_file = "run_batch_TravelTimeMatrix_%s.sh" %version
